...
object TagEditor {
    ...
    fun saveTags(context: Context, db: MusicDatabase, trackId: Long, title: String, artist: String, album: String): Result {
        ...
        if (track.displayName.endsWith(".mp3", ignoreCase = true)) {
            fileMessage = try {
                writeMp3Tags(context, Uri.parse(track.uri), cleanTitle, cleanArtist, cleanAlbum)
        ...
        db.updateTags(trackId, cleanTitle, cleanArtist, cleanAlbum)
        return Result(true, fileMessage)
    }

    private fun writeMp3Tags(context: Context, uri: Uri, title: String, artist: String, album: String) {
